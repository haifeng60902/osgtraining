...
GeometryTexturePatch32::GeometryTexturePatch32( int x , int y , int sizeC , int scaleC , osg::ref_ptr<osg::Image> image , double dAdd , double dScale )
{
...
GeometryTexturePatch32::~GeometryTexturePatch32()
{

}

osg::ref_ptr<osg::Vec3Array> GeometryTexturePatch32::CreateVertexArray( int x , int y , int sizeC , int scaleC )
{
	//создать массив вершин
...
osg::ref_ptr<osg::Vec2Array> GeometryTexturePatch32::CreateTexCoordArray( int x , int y , int sizeC , 
																		 int scaleC , unsigned char *dataR )
{
	//создать массив текстурных координат
	osg::ref_ptr<osg::Vec2Array> tc0 = new osg::Vec2Array;

	double kof = (double)scaleC / (double)( sizeC - 64.0 );

	//смещения текстурных координат
	std::pair< unsigned char , unsigned char > texOffset[64][64];
	...
	for (int i = 0 ; i < sizeC ; ++i )
	{
		...
		//расчет смещения текстурных координат( 64 раза от 0..1 )
...
void GeometryTexturePatch32::FillIndexVector( std::vector< unsigned int > &_vIndex , int sizeC )
{
	//заполнить вектор индексами
	_vIndex.push_back( 0 );
