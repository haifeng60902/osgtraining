#include "luaParseConfClient.h"

luaParseConfClient::luaParseConfClient()
{
...
void luaParseConfClient::parse(const char* conf, binClient* client)
{
...
void luaParseConfClient::extractCoins(Lua::Config* conf, tVecLuaCoin* pCoins)
{
	if (conf->open("coins"))
	{
		conf->iterate_begin();
...
void luaParseConfClient::extractMiners(Lua::Config* conf, tMapMiner* pMiners)
{
	if (conf->open("miners"))
	{
		conf->iterate_begin();
