...
GeometryTexturePatch4::GeometryTexturePatch4( int x , int y , int sizeC , int scaleC , osg::ref_ptr<osg::Image> image , double dAdd , double dScale )
{
...
GeometryTexturePatch4::~GeometryTexturePatch4()
{

}

osg::ref_ptr<osg::Vec3Array> GeometryTexturePatch4::CreateVertexArray( int x , int y , int sizeC , int scaleC )
{
	//создать массив вершин
...
osg::ref_ptr<osg::Vec2Array> GeometryTexturePatch4::CreateTexCoordArray( int x , int y , int sizeC , 
																		int scaleC , unsigned char *dataR )
{
	//создать массив текстурных координат
	osg::ref_ptr<osg::Vec2Array> tc0 = new osg::Vec2Array;

	double kof = (double)scaleC / (double)( sizeC - 8.0 );

	//смещения текстурных координат
	std::pair< unsigned char , unsigned char > texOffset[8][8];
	...
	for (int i = 0 ; i < sizeC ; ++i )
	{
		...
		//расчет смещения текстурных координат( 8 раза от 0..1 )
		...
		for ( int j = 0 ; j < sizeC ; ++j )
		{
			...
			//расчет смещения текстурных координат( 4 раза от 0..1 )
...
void GeometryTexturePatch4::FillIndexVector( std::vector< unsigned int > &_vIndex , int sizeC )
{
	//заполнить вектор индексами
	_vIndex.push_back( 0 );
