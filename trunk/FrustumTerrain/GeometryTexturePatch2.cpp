...
GeometryTexturePatch2::GeometryTexturePatch2( int x , int y , int sizeC , int scaleC , osg::ref_ptr<osg::Image> image , double dAdd , double dScale )
{
...
GeometryTexturePatch2::~GeometryTexturePatch2()
{

}

osg::ref_ptr<osg::Vec3Array> GeometryTexturePatch2::CreateVertexArray( int x , int y , int sizeC , int scaleC )
{
	//создать массив вершин
...
osg::ref_ptr<osg::Vec2Array> GeometryTexturePatch2::CreateTexCoordArray( int x , int y , int sizeC , 
																		int scaleC , unsigned char *dataR )
{
	//создать массив текстурных координат
	osg::ref_ptr<osg::Vec2Array> tc0 = new osg::Vec2Array;

	double kof = (double)scaleC / (double)( sizeC - 4.0 );

	//смещения текстурных координат
	std::pair< unsigned char , unsigned char > texOffset[4][4];
...
void GeometryTexturePatch2::FillIndexVector( std::vector< unsigned int > &_vIndex , int sizeC )
{
	//заполнить вектор индексами
	_vIndex.push_back( 0 );
