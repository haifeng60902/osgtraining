...
osg::ref_ptr<osg::Vec3Array> GeometryTexturePatch16::CreateVertexArray( int x , int y , int sizeC , int scaleC )
{
	//создать массив вершин
...
osg::ref_ptr<osg::Vec2Array> GeometryTexturePatch16::CreateTexCoordArray( int x , int y , int sizeC , 
																		int scaleC , unsigned char *dataR )
{
	//создать массив текстурных координат
	osg::ref_ptr<osg::Vec2Array> tc0 = new osg::Vec2Array;

	double kof = (double)scaleC / (double)( sizeC - 32.0 );

	//смещения текстурных координат
	std::pair< unsigned char , unsigned char > texOffset[32][32];
	...
	for (int i = 0 ; i < sizeC ; ++i )
	{
		...
		//расчет смещения текстурных координат( 32 раза от 0..1 )
...
void GeometryTexturePatch16::FillIndexVector( std::vector< unsigned int > &_vIndex , int sizeC )
{
	//заполнить вектор индексами
	_vIndex.push_back( 0 );
