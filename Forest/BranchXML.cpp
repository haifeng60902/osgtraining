...
void BranchXML::InitManyBranchGeode()
{
	//инициировать корневой узел данными со многими стволами
	...
	for ( int iY = 0 ; iY < 64 ; ++iY )
		for ( int iX = 0 ; iX < 64 ; ++iX )
		{
			...
			// Create an array of normals.
			osg::ref_ptr<osg::Vec3Array> n = new osg::Vec3Array;
			osg::Vec3 normal( 1.0 / 128.0 + iX * 1.0 / 64.0 ,
				1.0 / 128.0 + iY * 1.0 / 64.0 , 0 );
			n->push_back( normal );
			...
			// Create a Vec2Array of texture coordinates for texture unit 1
			// and attach it to the geom.
			...
			for ( int i = 0 ; i < _data.m_vCoords.size() / 3 ; ++i )
			{
				...
				//в текстурных координатах 1 будет нормаль
				osg::Vec3 tex1( _data.m_vNormals[ i * 3 ] , 
					_data.m_vNormals[ i * 3 + 1 ] ,
					_data.m_vNormals[ i * 3 + 2 ] );
